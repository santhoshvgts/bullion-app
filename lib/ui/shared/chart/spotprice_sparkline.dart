...
class SpotPriceSparkLineView extends StatelessWidget {
  ...
  const SpotPriceSparkLineView(this.metalName, this.chartData, {super.key,  this.loading = false});

  List<double> get d {
    List<ChartData>? filter = [];

    final releaseDateMap = chartData?.groupListsBy((m) => m.time.toString().split(':')[0]);
    ...
    return Container(
      ...
      child: loading! ? Sparkline(
       ...
       fillColor: const Color(0x40F2F2F7),
        ...
        fillColor: AppColor.opacityMetalColor(metalName!).withOpacity(0.2),
        fillMode: FillMode.below,
      ),
    );
  }
