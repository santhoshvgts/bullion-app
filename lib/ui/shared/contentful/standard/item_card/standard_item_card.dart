...
class StandardItemCard extends StackedView<StandardItemCardViewModel> {
  ...
  Widget builder(BuildContext context, StandardItemCardViewModel viewModel,
    ...
    return InkWell(
      ...
      child: Container(
        padding: EdgeInsets.all(viewModel.itemDisplaySettings!.fullBleed ||
                viewModel.itemDisplaySettings!.imagePosition == "top"
            ? 0
            : viewModel.itemDisplaySettings!.cardPadding),
        margin: EdgeInsets.only(
            bottom: viewModel.itemDisplaySettings!.hasBoxShadow ? 3.5 : 0),
        decoration: BoxDecoration(
            color: viewModel.itemDisplaySettings!.backgroundColor,
            boxShadow: viewModel.itemDisplaySettings!.hasBoxShadow
                ? AppStyle.cardShadow
                : null,
            borderRadius: viewModel.itemDisplaySettings!.hasRoundedCorners
                ? BorderRadius.circular(10)
                : null,
            border: viewModel.itemDisplaySettings!.hasBorders
            ...
            if (viewModel.item!.imageUrl != null)
              Align(
                alignment: UIAlignment.alignment(
                    viewModel.itemDisplaySettings!.imagePosition),
                child: _ImageContainer(
                  ...
                  child: NetworkImageLoader(
                    ...
                    borderRadius: UIAlignment.imageShapeRadius(
                        viewModel.itemDisplaySettings!.imageShape),
                      ...
                      if (viewModel.item!.subtitle != null &&
                        ...
                        Container(
                          ...
                          child: Text(viewModel.item!.subtitle!,
                              style: StandardTextStyle.subtitle(
                                  viewModel.itemDisplaySettings!.titleStyle,
                                  color: viewModel
                                      .itemDisplaySettings!.textColor)),
                        ),
                      if (viewModel.item!.content != null &&
                          viewModel.item!.content != "")
                        _ContentView()
                    ],
                  ),
                  if (viewModel.item!.actions != null)
                    if (viewModel.item!.actions!.isNotEmpty)
                      Container(
                        alignment: UIAlignment.alignment(
                            viewModel.itemDisplaySettings!.actionsAlignment),
                        margin: const EdgeInsets.only(top: 0),
                        child: Wrap(
                          alignment: UIAlignment.wrapAlignment(
                              viewModel.itemDisplaySettings!.actionsAlignment),
                          children: viewModel.item!.actions!
                              .map((actionItem) => ActionButtonItem(
                                    settings: actionItem,
                                    itemDisplaySettings:
                                        viewModel.itemDisplaySettings,
                                    textColor: viewModel
                                        .itemDisplaySettings!.textColor,
                                  ))
                              .toList(),
  ...
  Widget build(BuildContext context, StandardItemCardViewModel viewModel) {
    switch (viewModel.item!.contentType) {
      ...
      case ContentType.markdown:
        return ApmexHtmlWidget(
          MarkDown.markdownToHtml(viewModel.item!.content!),
        );
      ...
      default:
        return Container(
          alignment: UIAlignment.alignment(
              viewModel.itemDisplaySettings!.contentAlignment),
          child: Text(
            viewModel.item!.content!,
...
class _StandardDynamicWrapper extends StatelessWidget {
  ...
  _StandardDynamicWrapper(
      {this.key, this.contentKey, this.imagePosition, this.children})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    if ((imagePosition!.contains("right") || imagePosition!.contains("left")) &&
...
class _StandardDynamicContentWrapper
    extends ViewModelWidget<StandardItemCardViewModel> {
  ...
  _StandardDynamicContentWrapper(
      {this.key, this.imagePosition, this.actionPosition, this.children})
      : super(key: key);
  ...
  Widget build(BuildContext context, StandardItemCardViewModel viewModel) {
    return Container(
      margin: EdgeInsets.only(
          top: (imagePosition!.contains("right") ||
                      imagePosition!.contains("left")) &&
                  imagePosition != "center"
              ? 0
              : 5,
          left: imagePosition == "left" ? 10 : 0),
      padding: EdgeInsets.all(imagePosition == "top"
          ? viewModel.itemDisplaySettings!.cardPadding
          : 0),
      child: actionPosition!.contains("right") ||
              actionPosition!.contains("left")
          ? Row(
              ...
              children: [
                ..._children
                        ...
                        .map((index, element) => MapEntry(
                              index,
                              index == 0 ? Expanded(child: element) : element,
                            ))
                        .values
                        .toList() ??
                    []
